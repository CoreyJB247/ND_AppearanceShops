Config = {
    {
        price = 1,
        text = "Change clothes",
        blip = {
            label = "Police Locker",
            sprite = 366,
            scale = 0.6,
            color = 38
        },
        appearance = {
            components = true,
            props = true,
        },
        locations = {
            { change = vec4(458.24, -998.52, 30.68, 357.12) },
            { change = vec4(1825.24, 3679.68, 34.4, 115.36) },
            { change = vec4(1543.64, 816.32, 77.64, 327.36) },
            { change = vec4(853.48, -1290.0, 26.72, 264.28) },
            { change = vec4(-438.08, 5989.0, 31.96, 136.08) },
            { change = vec4(-570.04, -105.04, 33.68, 109.6) },
        }
    },
    {
        price = 1,
        text = "Change clothes",
        blip = {
            label = "SAFR Locker",
            sprite = 366,
            scale = 0.6,
            color = 1
        },
        appearance = {
            components = true,
            props = true,
        },
        locations = {
            { change = vec4(300.64, -597.6, 43.28, 352.72) },
            { change = vec4(-360.68, 6114.72, 31.48, 51.84) },
            { change = vec4(1689.24, 3593.8, 35.72, 28.68) },
            { change = vec4(369.48, 282.96, 102.96, 248.12) },
            { change = vec4(-630.76, -86.24, 45.4, 167.88) },
            { change = vec4(321.64, -1012.8, 29.16, 186.48) },
            { change = vec4(-1036.08, -1391.04, 4.96, 250.48) },
        }
    },
    {
        price = 1,
        text = "Change clothes",
        blip = {
            label = "Clothing store",
            sprite = 73,
            scale = 0.8,
            color = 3
        },
        appearance = {
            components = true,
            props = true,
        },
        locations = {
            { change = vec4(11.10, 6513.75, 31.88, 44.84) },
            { change = vec4(1696.50, 4828.62, 42.06, 96.94) },
            { change = vec4(-1107.79, 2709.55, 19.11, 224.57) },
            { change = vec4(617.47, 2765.67, 42.09, 184.34) },
            { change = vec4(1191.30, 2713.67, 38.22, 184.49) },
            { change = vec4(-3175.01, 1042.75, 20.86, 334.28) },
            { change = vec4(-704.33, -151.64, 37.42, 190.64) },
            { change = vec4(-168.40, -299.32, 39.73, 293.06) },
            { change = vec4(121.85, -224.58, 54.56, 336.21) },
            { change = vec4(-1189.36, -769.34, 17.32, 128.69) },
            { change = vec4(428.58, -800.78, 29.49, 91.14) },
            { change = vec4(-828.94, -1073.56, 11.33, 212.48) },
            { change = vec4(72.54, -1398.72, 29.38, 272.27) },
            { change = vec4(-1447.16, -242.78, 49.82, 100.09) }
        }
    },
    {
        price = 100,
        text = "Get a tattoo",
        blip = {
            label = "Tattoo parlour",
            sprite = 75,
            scale = 0.8,
            color = 0
        },
        appearance = {
            tattoos = true
        },
        locations = {
            { change = vec4(319.2, 186.64, 99.88, 339.36) },
            { change = vec4(-3176.44, 1072.96, 17.12, 62.04) },
            { change = vec4(-1147.08, -1427.52, 1.32, 215.56) },
            { change = vec4(1329.68, -1653.12, 48.56, 219.2) },
            { change = vec4(-294.41, 6200.52, 31.49, 221.79) }
        }
    },
    {
        price = 15,
        text = "Get a haircut",
        blip = {
            label = "Barber shop",
            sprite = 71,
            scale = 0.8,
            color = 0
        },
        appearance = {
            headOverlays = true
        },
        locations = {
            { change = vec4(-277.82, 6226.55, 31.70, 314.70) },
            { change = vec4(-814.23, -182.45, 37.57, 211.02) },
            { change = vec4(-34.45, -153.40, 57.09, 251.27) },
            { change = vec4(1213.55, -474.17, 66.22, 348.66) },
            { change = vec4(-1281.34, -1118.29, 7.00, 358.13) },
            { change = vec4(138.70, -1707.60, 29.30, 48.86) }
        }
    }

}
